...
.get('/matchInfo/:rg/:summonerName/', async (req, res) => {
  ...
  /*
  let st = new Date()
  if (Object.values(liveMatches).length) if (Object.values(liveMatches).find(match => match.matchInfo.participants.find(summ => summ.summonerName.toLowerCase() == req.params.summonerName.toLowerCase()))) {
    let r = Object.values(liveMatches).find(match => match.matchInfo.participants.find(summ => summ.summonerName.toLowerCase() == req.params.summonerName.toLowerCase()))
    if (r.status != 5)
      return res.json({...r, load_time: Date.now() - st})
  }
  try {
  ...
  if (!liveMatches[gameId] || !liveMatches[gameId].status.match)
    updateLiveMatch(plf, gameId, {matchInfo, status: {match: true, leagues: false, stats: false, ended: false}})
  ...
  new Promise(async () => {
    ...
    try {
      var leagues = await Api.entriesBySummoner(plf, participants)
    } catch (e) {
      return res.status(e).json({response_code: e})
    }
    r = {team1: [], team2: [], matchInfo}
    matchInfo.participants.map((el, i) => 
      r['team' + el.teamId / 100].push({
        ...el,
    ...
    updateLiveMatch(plf, gameId, {...r, status: {...liveMatches[gameId].status, leagues: true}})
    let update = setInterval(async () => {
      try {
        await Api.activeGame(plf, summoner.id)
      } catch (e) {
        if (e == 404) {
          clearInterval(update)
          updateLiveMatch(plf, gameId, await {...Api.matches(plf, matchInfo.gameId), status: {...liveMatches[gameId].status, ended: true}})
          setTimeout(() => deleteLiveMatch(plf, matchInfo.gameId), 300000)
        }
      }
    }, 5000)
    //liveMatches[matchInfo.gameId] = {status: 'cpl', ...r}
    //return res.json({response_code: 200, load_time: Date.now() - st, matchInfo: matchInfo, leagues, r})
  })
  */
})
